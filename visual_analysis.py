...
class VisualSentiment(object):
    ...
    #             args.font_path = './fonts'
    #         if not os.path.exists(args.font_path):
    #             os.makedirs(args.font_path)
    ...
    def __init__(self, font_path=None):
        ...
        if not os.path.exists(os.path.join(save_dir, default_name + ".ttf")):
            download_file(save_dir, default_name + ".ttf", URLS[default_name][0], URLS[default_name][1])

        self._font = os.path.join(save_dir, default_name + ".ttf")

        self.wc = wordcloud.WordCloud(font_path=self._font, background_color="white", width=800, height=400)
        plt.figure(figsize=(8, 6))
    ...
    def _plot_histogram(
        ...
        # 创建一个 FontProperties 对象，并设置字体文件路径
        ...
        plt.xlabel(plt_xlabel,fontproperties=font_prop)
        plt.ylabel(plt_ylabel,fontproperties=font_prop)
